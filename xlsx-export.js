...
(function (global) {
  ...
  function zipStore(files) {
    ...
    for (const f of files) {
      ...
      chunks.push(lh);
      // central directory record (46 bytes fixed: sig+made+need+flag+method+time+date
      //   +crc+csize+usize+fnlen+extralen+commentlen+disk+internal+external+offset, then name)
      const cd = concat([
